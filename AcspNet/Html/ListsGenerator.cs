...
namespace AcspNet.Html
{
	...
	public sealed class ListsGenerator : IListsGenerator
	{
		...
		internal ListsGenerator(IStringTable stringTable)
		{
		...
		{
			return string.Format("<option value=''{1}>{0}</option>", _stringTable["HtmlListDefaultItemLabel"], isSelected ? " selected='selected'" : "");
		}
	}
}